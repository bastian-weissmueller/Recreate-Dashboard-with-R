plot_cumulative_cases_ranking <- function(data, Country=NULL) {
  ...
  top_countries <- data %>%
    ...
    arrange(desc(cumulative_total_cases))
  ...
  # Format cumulative_total_cases with thousands separators
  top_countries <- top_countries %>%
    mutate(cumulative_total_cases_formatted = format(cumulative_total_cases, big.mark = ","))
  
  # Create the Plotly bar chart
  p <- plot_ly(
    data = top_countries,
    x = ~cumulative_total_cases,
    y = ~reorder(country, cumulative_total_cases),
    type = 'bar',
    orientation = 'h',
    ...
    text = ~cumulative_total_cases_formatted,  
