plot_daily_cases_ranking <- function(data, Country=NULL) {
  ...
  # Format daily_new_cases with thousands separators
  top_countries <- top_countries %>%
    mutate(daily_new_cases_formatted = format(daily_new_cases, big.mark = ","))
  
  # Create the Plotly bar chart
  p <- plot_ly(
    data = top_countries,
    x = ~daily_new_cases,
    y = ~reorder(country, daily_new_cases),
    type = 'bar',
    orientation = 'h',
    ...
    text = ~daily_new_cases_formatted,  
