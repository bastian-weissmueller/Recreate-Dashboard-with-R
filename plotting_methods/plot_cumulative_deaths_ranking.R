plot_cumulative_deaths_ranking <- function(data, Country=NULL) {
  ...
  # Format cumulative_deaths with thousands separators
  top_countries <- top_countries %>%
    mutate(cumulative_deaths_formatted = format(cumulative_deaths, big.mark = ","))
  
  # Create the Plotly bar chart
  p <- plot_ly(
    data = top_countries,
    x = ~cumulative_deaths,
    y = ~reorder(country, cumulative_deaths),
    type = 'bar',
    orientation = 'h',
    ...
    text = ~cumulative_deaths_formatted,  
